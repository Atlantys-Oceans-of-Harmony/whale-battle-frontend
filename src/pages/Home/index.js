...
const GAME_STATES = [
  { title: "Create Battle", index: 0 },
  { title: "Join Battle", index: 1 },
];
const Home = () => {
  ...
  useEffect(() => {
    const fetchStuff = async () => {
      ...
      const idsToSend = [...new Set([...listOfWhales, ...allWhales])].filter(
        (e) => e !== "0"
      );
  ...
  const handleOnCreateBattle = async (data) => {
    ...
    if (
      battlesToJoin.filter((e) => e.battleId === data.battleId).length === 0
    ) {
  ...
  const handleOnCancelledBattle = async (data) => {
    setBattlesToJoin((battlesToJoin) =>
      battlesToJoin.filter((e) => e.battleId !== data.battleId)
    );
    setCreatedBattles((createdBattles) =>
      createdBattles.filter((e) => e.battleId !== data.battleId)
    );
  };
  ...
  const handleOnCreateOwnBattle = async (data) => {
    if (
      createdBattles.filter((e) => e.battleId === data.battleId).length === 0
    ) {
  ...
  const [createBattleSelectedIndex, setCreatedBattleSelectedIndex] =
    useState(-1);
    ...
    return (
      <div className="">
        <BattleToJoinModal
          ...
          {...(createBattleSelectedIndex >= 0
            ? createdBattles[createBattleSelectedIndex]
            : {})}
        />
        ...
        <div className="shadow sm:rounded-md sm:overflow-hidden">
          <div className=" py-6 px-4 space-y-6 sm:p-6">
            ...
            <div className="lg:flex justify-center">
              <div>
                {harmonyWhales && (
                  <>
                    <h3 className="text-white sm:text-xl md:text-2xl font-extrabold">
                      Select a Whale
                    </h3>
                    ...
                    <div className="h-32 mt-4 -top-6 -left-8 w-96 relative text-md tracking-tight font-extrabold text-white sm:text-xl md:text-2xl flex items-center wallet-btn ">
                      <select
                        ...
                        onChange={(e) =>
                          handleCreateBattleChange("whaleId", e.target.value)
                        }
                      >
                        {harmonyWhales.map((e) => {
                          return (
                            <option className="flex bg-blue-600" value={e}>
                              Whale #{e}
                            </option>
                          );
                        })}
                      ...
                      <img
                        src={Button1}
                        alt=""
                        className="absolute -z-10 w-64 -top-10 -left-10"
                      />
                    </div>
                    <div className="relative z-10 mb-4 text-white text-xl ml-4 font-bold">
                      Win Rate:{" "}
                      {
                        whaleData[createBattleForm?.whaleId?.toString()]
                          ?.percent
                      }
                      % (
                      {whaleData[createBattleForm?.whaleId?.toString()]?.wins}/
                      {whaleData[createBattleForm?.whaleId?.toString()]?.loses})
                    </div>

                    <div className="relative w-80 mt-4">
                      <img src={Frame} alt="" className="absolute w-96" />
                      <img
                        src={`https://gen1.atlantys.one/token/image/${createBattleForm?.whaleId}`}
                        alt=""
                        className="pt-12 ml-8 w-64"
                      />
                    </div>
                  </>
                )}
                ...
                <div className="text-white sm:text-xl md:text-2xl font-extrabold">
                  ...
                  <div className="h-32 mt-4 mx-auto  -top-6 -left-8 w-64 relative text-md tracking-tight font-extrabold text-white sm:text-xl md:text-2xl flex items-center ">
                    <input
                      ...
                      onChange={(e) =>
                        handleCreateBattleChange("amount", e.target.value)
                      }
                    />
                  </div>
          ...
          <div className="h-32  cursor-pointer mx-auto  w-80 relative text-md tracking-tight font-extrabold text-white sm:text-xl md:text-2xl flex items-center wallet-btn ">
            <button
        ...
        <div className="relative z-10">
          <div className="text-2xl text-center font-bold mb-8 text-white">
            Battles Created
          </div>
          <div className="text-white w-full text-md text-right pr-16">
            * Win % (Battles Won)/(Battles Lost)
          </div>
          ...
          {createdBattles?.length > 0 ? (
            ...
            <div className="mx-auto pb-16  w-full text-center text-xl text-white">
          ...
          {createdBattles?.length > 0 ? (
            <div className="w-full">
              <div
                onClick={() =>
                  cancelAllBattles(createdBattles?.map((e) => e.battleId))
                }
              ...
              >
                <button className="w-full font-extrabold text-lg cursor-pointer flex items-center justify-center px- py-3 mb-3 ">
                  Cancel All Battles
                </button>
              </div>
            </div>
    ...
    return (
      <div>
        <BattleToJoinModal
          ...
          {...(joinBattleSelectedIndex >= 0
            ? battlesToJoin[joinBattleSelectedIndex]
            : {})}
        />
              ...
              <>
                <div className="h-32 ml-10 mt-4 -top-6 -left-8 w-96 relative text-md tracking-tight font-extrabold text-white sm:text-xl md:text-2xl flex items-center wallet-btn ">
                  <select
                    ...
                    onChange={(e) =>
                      handleJoinBattleChange("whaleId", e.target.value)
                    }
                  >
                    {harmonyWhales.map((e) => {
                      return (
                ...
                <div className="relative z-10 mb-4 text-white text-xl font-bold">
                  Win Rate:{" "}
                  {whaleData[joinBattleForm?.whaleId?.toString()]?.percent}% (
                  {whaleData[joinBattleForm?.whaleId?.toString()]?.wins}/
                  {whaleData[joinBattleForm?.whaleId?.toString()]?.loses})
                </div>

                <div className="relative w-80 m-4">
                  <img src={Frame} alt="" className="absolute w-96" />
                  <img
                    src={`https://gen1.atlantys.one/token/image/${joinBattleForm?.whaleId}`}
                    alt=""
                    className="pt-12 ml-8 w-64"
                  />
                </div>
        ...
        <h3 className=" text-center text-3xl my-8 text-white leading-6 font-bold">
          Battles to join appear here:
        </h3>
        <div className="text-white w-full text-md text-right pr-16">
          * Win % (Battles Won)/(Battles Lost)
        </div>
        ...
        <div className="px-16 lg:px-8 grid md:grid-cols-2 lg:grid-cols-4 grid-cols-1 max-w-7xl mx-auto">
          {battlesToJoin?.length > 0 ? (
            battlesToJoin.map((e, index) => {
              const handleClick = () => {
  ...
  if (!correctChain) {
    return (
      <div className="flex flex-col text-white items-center justify-center h-screen">
        ...
        <div className="w-full">
          <div className="mx-auto h-36 w-96 relative text- tracking-tight font-extrabold focus-visible:border-0 focus:border-0 text-white sm:text-xl md:text-2xl flex items-center wallet-btn ">
            <button
              onClick={() => {
                promptChain(true);
              }}
    ...
    return (
      <div className="flex flex-col items-center justify-center h-screen">
        <div className="text-4xl text-white font-bold">
          You need to have Gen1 Whales to battle!
        </div>
      </div>
    );
  }
  return (
    <>
      <WonSummary {...wonSummary} open={openWon} setOpen={setOpenWon} />
      ...
      <div className="w-full">
        <div className="flex w-full mt-8 justify-between items-center">
          ...
          <div className=" h-36 w-96 relative text- tracking-tight font-extrabold focus-visible:border-0 focus:border-0 text-white sm:text-xl md:text-2xl flex items-center wallet-btn ">
            <button
              onClick={() => {
                setGameState(GAME_STATES[(gameState.index + 1) % 2]);
              }}
              className="w-full font-extrabold text-lg cursor-pointer flex items-center justify-center px- py-3 mb-3 "
            >
                  ...
                  return (
                    <>
                      <BattleProgressCard
                        ...
                        {...e}
                        isComplete={
                          parseInt(e.futureBlock) - parseInt(blockNumber) <= 0
                        }
                      />
                    </>
                  );
